...
// 🚀 NEW: Admin Access Control Table 🚀
const BIGQUERY_ADMIN_TABLE = "scheduler_ui_admins"; 
...
// 🎯 DYNAMIC CACHE: This replaces the hardcoded list 🎯
let ADMIN_EMAILS_BACKEND_CACHE = []; 
const SYSTEM_EMAIL_FOR_GLOBAL_TASKS = "systems@brightbraintech.com";


// 🎯 CORRECTED FUNCTION: Fetch Admin Emails from BigQuery using 'Emails' and 'Access' columns 🎯
async function fetchAdminEmails() {
    ...
    try {
        ...
        // CRITICAL: Throw an error to halt the server startup if this essential list can't load.
        ADMIN_EMAILS_BACKEND_CACHE = [];
        throw new Error('Failed to load critical admin access list. Check BigQuery table schema/permissions.');
    }
}
...
// 🎯 NEW ENDPOINT: Provide the dynamic admin list to the frontend 🎯
app.get('/api/admin-emails', (req, res) => {
    res.status(200).json(ADMIN_EMAILS_BACKEND_CACHE);
});
...
app.get('/api/data', async (req, res) => {
    ...
    // 🎯 UPDATED CHECK: Uses the dynamic cache 🎯
    if (userEmail && !ADMIN_EMAILS_BACKEND_CACHE.includes(userEmail)) {
    ...
    // 🎯 UPDATED CHECK: Uses the dynamic cache 🎯
    } else if (userEmail && ADMIN_EMAILS_BACKEND_CACHE.includes(userEmail)) {
...
app.post('/api/post', async (req, res) => {
    ...
    try {
        ...
        if (currentTask && currentTask.Responsibility !== mainTask.Responsibility) {
            ...
            // 🎯 UPDATED CHECK: Uses the dynamic cache 🎯
            const isAdmin = ADMIN_EMAILS_BACKEND_CACHE.includes(userEmail);
...
// 🎯 UPDATED: Execute the async fetch function before starting the server 🎯
fetchAdminEmails().then(() => {
    app.listen(PORT, () => {
        console.log(`Server running on port ${PORT}`);
        console.log(`Application is ready to handle requests.`);
    });
}).catch(err => {
    console.error('Server failed to start due to Admin Email fetch error:', err);
